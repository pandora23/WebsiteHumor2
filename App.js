...
function pullAllDataUsingSiteList(urlList, whichQuery){
	...
	getSiteData(urlList, 0, whichQuery);
...
function getSiteData(urlX, count, whichList){
	

	console.log("ZZZ" + urlX[count]);
	...
	if(enough == false){
		if(urlX[count] != ""){
				...
				data : {texts : urlX[count], which: whichList, AmbiguousWord: A, dataInitialized: initialized, idNum: idNumber, dist: d},
				success: function(data){
					idNumber=data['idNum'];
					console.log("IDNUM: " + idNumber);
					console.log(data)
					...
					getSiteData(urlX, count+1, whichList);
